...
| albIngress | object | `{"enabled":false,"nginxServiceName":"nginx-ingress-ingress-nginx-controller"}` | ALB Ingress configuration (for nginx ingress flow) When using nginx ingressClass, this creates an ALB Ingress in ingress-nginx namespace |
...
| worker | object | `{"replicaCount":1,"resources":{},"sharedStorage":false}` | Worker configuration (only used when Redis is enabled for queue mode) |
| worker.replicaCount | int | `1` | Number of worker replicas |
| worker.resources | object | `{}` | Worker resource limits and requests (defaults to main resources if not specified) |
| worker.sharedStorage | bool | `false` | Whether workers should mount shared storage (requires ReadWriteMany if enabled) Workers typically don't need persistent storage as they only execute jobs |
